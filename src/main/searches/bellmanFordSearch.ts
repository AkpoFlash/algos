...
/**
 * Bellman-Ford algorithm finds the shortest path to all vertices in directed edge-weigthed graph
 * This algorithm also works with negative weigth
 *
 * How it's work:
 * - Set all distances to infinity except start vertice which set to zero
 * - Start from 'Start Vertice'
 * - Run over all adjacent edges V times and relax each of them
 *   (relaxation is method for recalculating distance to vertice)
 */
export class BellmanFordSearch<T extends string | number | symbol> {
    ...
    constructor(graph: EdgeWeightedDigraph<T>, startVertice: T) {
        ...
        for (let i = 0; i < graph.vertices.length; i++) {
            for (let j = 0; j < graph.vertices.length; j++) {
                const adjacentEdges = graph.adjacentEdges(graph.vertices[j]) || [];
                for (let e = 0; e < adjacentEdges.length; e++) {
                    this._relax(adjacentEdges[e]);
                }
            }
        }
    }
...
// const shortPath = new BellmanFordSearch(graph, 0);
